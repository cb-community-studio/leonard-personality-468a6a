...
import responsesFakerFactory from "./responsesFakerFactory";
...
const ResponsesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = responsesFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("responses").create(elem));
...
export default connect(null, mapDispatch)(ResponsesFakerDialogComponent);
